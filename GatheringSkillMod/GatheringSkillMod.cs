...
{
    [BepInPlugin("com.pipakin.GatheringSkillMod", "GatheringSkillMod", "2.0.0")]
    ...
    public class GatheringSkillMod : BaseUnityPlugin
    {
        ...
        private static GatheringConfig gatheringConfig = new GatheringConfig();
        ...
        void Awake()
        {
            ...
            gatheringConfig.InitConfig("com.pipakin.GatheringSkillMod", Config);
        ...
        {
            var ignored = gatheringConfig.IgnorePickables;
            if(ignored != null)
            {
        ...
        public static void IncreaseSkill(Character character, string pickableName)
        {
            try
            {
                ...
                else
                {
                    ...
                    character.RaiseSkill((Skills.SkillType)SKILL_TYPE, gatheringConfig.SkillIncrease);
                }
            }
            catch (Exception e)
            {
        ...
        public static class GetHoverTextPatch
        {
            ...
            {
                if (___m_picked && ___m_nview.GetZDO() != null && gatheringConfig.EstimatesEnabled)
                {
                    ...
                    if (level >= gatheringConfig.DetailedEstimateLevel)
                    {
                    ...
                    else if (level >= gatheringConfig.SimpleEstimateLevel)
                    {
        ...
        public static class AddDropMultiplier
        {
            ...
            {
                if (!___m_nview.IsValid() || ___m_picked || !gatheringConfig.DropsEnabled || IgnorePickable(__instance.name))
                {
                    return;
                }

                var mult = 1.0f + character.GetSkillFactor((Skills.SkillType)SKILL_TYPE) * ((float)gatheringConfig.MaxDropMultiplier - 1.0f);
        ...
        public static class DropMultiply
        {
            ...
            {
                if (!___m_nview.IsValid() || !gatheringConfig.DropsEnabled || IgnorePickable(__instance.name))
                {
                    return;
                }
